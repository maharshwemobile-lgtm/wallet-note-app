...
export async function getBalances(agentId: string) {
  ...
  const approved = rows.filter(r => r.agentId === agentId && r.status === "Approved");
...
export async function recordWalletTransaction(input: {
  agentId: string; type: WalletTxType; currency: Currency; amount: number; status?: string; referenceType?: string; referenceId?: string; note?: string;
}) {
  ...
  await appendRow("WalletTransactions!A:J", [id, new Date().toISOString(), input.agentId, input.type, input.currency, input.amount, signedAmount, status, input.referenceType ?? "", input.referenceId ?? input.note ?? ""]);
  return id;
}
