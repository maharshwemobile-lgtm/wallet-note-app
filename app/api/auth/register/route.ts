...
export async function POST(req: Request) {
  ...
  if (users.some((user) => user.email?.toLowerCase() === email)) return NextResponse.json({ error: "Email is already registered" }, { status: 409 });
  if (users.some((user) => user.username?.toLowerCase() === username)) return NextResponse.json({ error: "Username is already taken" }, { status: 409 });
  ...
  const res = NextResponse.json({ ok: true, next: "/connect-sheet" });
