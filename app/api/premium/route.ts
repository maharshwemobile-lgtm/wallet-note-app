  ...
  try {
    ...
    if (body.action === "settleLotteryDealer") {
      ...
      const settled = await transaction(async (client) => {
        ...
        const totalResult = await client.query(`SELECT COALESCE(SUM(bet_amount),0) total, COUNT(*) count FROM lottery_entries WHERE user_id=$1 AND draw_date=$2 AND lottery_type=$3 AND currency=$4 AND dealer_status='pending' AND deleted_at IS NULL FOR UPDATE`, [session.userId, parsed.data.date, parsed.data.type, parsed.data.currency]);
        const amount = num(totalResult.rows[0].total);
